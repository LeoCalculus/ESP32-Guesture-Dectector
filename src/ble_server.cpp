...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_ADV_START_COMPLETE_EVT:
        ...
        ESP_LOGI(GATTS_TABLE_TAG, "Advertising start successfully - Device should be visible as ESP_SPP_SERVER");
        break;
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    switch (event) {
    	...
    	case ESP_GATTS_CONNECT_EVT:
    	    {
            ESP_LOGI(GATTS_TABLE_TAG, "BLE Client connected successfully, conn_id %u, remote "ESP_BD_ADDR_STR"",
...
void send_message_to_client(const char* message){
    if (!is_connected || !enable_data_ntf) {
        ESP_LOGW(GATTS_TABLE_TAG, "Not connected or notification not enabled");
        return;
    }
    uint16_t len = strlen(message);
    esp_ble_gatts_send_indicate(spp_gatts_if, spp_conn_id, spp_handle_table[SPP_IDX_SPP_DATA_NTY_VAL], len, (uint8_t *)message, false);
}
